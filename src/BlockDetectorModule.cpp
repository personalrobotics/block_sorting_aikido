...
#include <tabletop_perception_tools/msg/Block.h>
#include <stdexcept>
#include <utility>
...
std::vector<tabletop_perception_tools::Block> BlockDetectorModule::findBlocksFromService(const string& serviceName,
																					...
																					vector<float> box_min={-0.3, -0.05, 0.6},
																					vector<float> box_max={0.3, 0.5, 1.5}){
	...
	ros::ServiceClinet client = mNode.serviceClient<tabletop_perception_tools::FindBlocks>(serviceName);
	tabletop_perception_toos:FindBlocks srv;
	srv.request.point_cloud_topic = cloud_topic;
	srv.request.segment_planes = segment_planes;
	srv.request.num_planes = num_planes;
	srv.reqeust.plane_distance = plane_distance;
...
std::vector<dart::dynamics::SkeletonPtr> BlockDetectorModule::detectObjects(const dart::simulation::WorldPtr& env, 
	...
	for(auto const& block : detectedBlocks){
		...
		blockSkeletonPtr = makeBodyFromURDF(mBlockUri, skel_pose);
		...
		if(env_skeleon != nullptr){
			env->removeSkeleon(env_skeleton);
		}
